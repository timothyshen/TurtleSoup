...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    logError({ endpoint: "generate-review", uid: auth.token.uid, code: "upstream_unreachable", message: msg });
...
async function handleStreaming(apiKey: string, userPrompt: string, uid: string): Promise<Response> {
  ...
  try {
    ...
    const msg = (e as Error).message;
    logError({ endpoint: "generate-review", uid, code: "upstream_unreachable", message: msg, extra: { mode: "stream" } });
    return withCORS(jsonError(502, "upstream_unreachable", `Failed to reach Anthropic: ${msg}`));
  }
